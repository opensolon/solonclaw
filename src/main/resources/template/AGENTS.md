...
- 子任务执行过程中应主动使用 `report_progress` 上报当前阶段和最新进展，便于父任务向用户同步更具体的状态，而不是只说“处理中”。
- 当用户要求停止后台任务时，使用 `cancel_task` 取消正在运行或排队中的子任务。
- 当用户要求修正方向、补充要求或追加关注点时，使用 `append_instruction` 将新指令追加给对应子任务。
